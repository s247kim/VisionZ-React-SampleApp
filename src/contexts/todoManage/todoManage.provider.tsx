...
export const useTodoManageAction = (): Dispatch<TodoManageAction> => {
  ...
  if (!context) {
    throw new Error(
      "useTodoManageAction must be used within a TodoManageProvider"
    );
  }
  return context;
};
...
export const useTodoManageStore = (): TodoManageStore => {
  ...
  if (!context) {
    throw new Error(
      "useTodoManageStore must be used within a TodoManageProvider"
    );
  }
  return context;
};
...
export const TodoManageProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  return (
    <TodoManageActionContext.Provider value={action}>
      <TodoManageStoreContext.Provider value={state}>
        {children}
      </TodoManageStoreContext.Provider>
